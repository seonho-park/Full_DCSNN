...
class ResNetUNet(nn.Module):
    ...
    def __init__(self, base_model, n_class = 1):
        ...
        self.conv_original_size0 = convrelu(3, 48, 3, 1)
        self.conv_original_size1 = convrelu(48, 48, 3, 1)
        self.conv_original_size2 = convrelu(48 + 128, 64, 3, 1)
...
def dcsnn_full(nglobal, pretrained = True, progress=True, **kwargs):
    ...
    # keypoint_generator = ResNetUNet(encoder)
    # net = DCSNN_Full(keypoint_generator, encoder_k, dim=nglobal, **kwargs)
    net = MoCo(encoder, encoder_k, dim=nglobal, **kwargs)
    return net
    # return _resnet('resnet50', nglobal, Bottleneck, [3, 4, 6, 3], pretrained, progress,
    #                **kwargs)

class DCSNN_Full(nn.Module):
    def __init__(self, keypoint_gen, encoder_k, **kwargs):
        super(DCSNN_Full, self).__init__()
        encoder = keypoint_gen.base_model
        self.moco = MoCo(encoder, encoder_k, **kwargs)
        self.keypoint_gen = keypoint_gen

    def forward(self, im_q, im_k=None):
        kp_q = self.keypoint_gen(im_q)
        if im_k is not None:
            kp_k = self.keypoint_gen(im_k)
            moco_out = self.moco(im_q, im_k)
            return [kp_q, kp_k], moco_out
        else:
            moco_out = self.moco(im_q)
            return kp_q, moco_out
